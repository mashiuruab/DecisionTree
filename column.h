...
public:
    ...
    string string_value(size_t row_idx) {
...
public:
	...
	{
		istringstream(value) >> missing_value;
	}
...
public:
	...
	virtual void append(size_t row_idx, void* value) {
		if(!row_index.empty()) assert(row_index.back() < row_idx); ///### debugging (row_idx check), can be removed in real deployment
	...
	{
		istringstream(value) >> default_value;
	}
