...
int main(int argc, char** argv){
    Matrix X;
    load_meta("meta_soybean_cases.csv", X);
    X.print_meta();
    return 0;
}
//*/
...
//- is_ordinal = true
//- is_dense = true
void load_meta(int num_cols, Matrix &X)
{
    for(size_t i = 0; i < num_cols; i++){ //traverse through columns
        X.append(new DenseColumn<double>(ELEM_DOUBLE, true));
    }
}
...
void load_csv(const char* datafile, int num_cols, Matrix &X)
{
    load_meta(num_cols, X);
    load_data(datafile, X);
}
...
int main(int argc, char** argv){
    Matrix X;
    load_csv("soybean.csv", "meta_soybean_latest.csv", X);
    X.print();
    return 0;
}
//*/
...
/*
int main(int argc, char** argv){
    // here the last column is string and
    // we can not convert string to double (here double is the default column type)
    Matrix X;
    load_csv("soybean.csv", 36, X);
    X.print();
    return 0;
}
//*/
...
int main(int argc, char** argv){
    ...
    cout << "string to double cast value = " << value << endl;
...
void shuffle_indices(size_t sample_size, size_t n_split, vector<vector<size_t>> &shuffled_indices) {
    ...
    int chunk = sample_size / n_split;
    int remain = sample_size % n_split;
    int offset = 0;
